<!-- KonaklamaTipiGrid.vue -->
<template>
  <div class="konaklama-tipi-grid">
    <q-table
...
// Data
const accommodationTypes = ref<AccommodationType[]>(ACCOMMODATION_TYPES)
...
// Reactive state
const selectedTypes = ref<string[]>(accommodationTypes.value.map(type => type.value))

// Computed
const selectAll = computed({
  get: () => selectedTypes.value.length === accommodationTypes.value.length,
  set: (val: boolean) => {
    if (val) {
      selectedTypes.value = accommodationTypes.value.map(type => type.value)
    } else {
      selectedTypes.value = []
    }
  }
...
const isIndeterminate = computed(() => {
  ...
  return selected > 0 && selected < accommodationTypes.value.length
})

const selectedTypeLabels = computed(() => {
  return selectedTypes.value.map(value => {
    const type = accommodationTypes.value.find(t => t.value === value)
...
.accommodation-table :deep(.q-table__top) {
  padding: 0;
}

.accommodation-table :deep(.q-table thead th) {
...
.accommodation-table :deep(.q-table tbody td) {
  padding: 4px 6px;
  border-bottom: 1px solid #f3f4f6;
}

.accommodation-table :deep(.q-table tbody tr:last-child td) {
  border-bottom: none;
}

.accommodation-table :deep(.q-table tbody tr:hover) {
...
/* Dark mode support */
.body--dark .accommodation-table :deep(.q-table thead th) {
  background-color: #374151;
  color: #f9fafb;
  border-bottom: 2px solid #4b5563;
}

.body--dark .accommodation-table :deep(.q-table tbody td) {
  border-bottom: 1px solid #4b5563;
}

.body--dark .accommodation-table :deep(.q-table tbody tr:hover) {
  background-color: #374151;
}
